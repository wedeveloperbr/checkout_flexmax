...
export default function Frete({ data, onUpdate, onNext }: FreteProps) {
  ...
  useEffect(() => {
    onUpdate({ ...formData, tipo: selectedShipping })
  ...
  }, [formData, selectedShipping, onUpdate])
  ...
  const isFormValid =
    Object.values(validFields).every((valid) => valid) && selectedShipping !== ""
  ...
  return (
    <div>
      ...
      {/* opções de frete */}
      {["PAC", "SEDEX", "Loggi"].map((opcao) => (
        <div
          key={opcao}
          className="checkbox mt-2 p-3"
          style={{
            border:
              selectedShipping === opcao
                ...
                : "1px solid #191F2D1A",
            background:
              selectedShipping === opcao ? "#41DA690D" : "#FFFFFF",
          }}
          onClick={() => setSelectedShipping(opcao)}
        >
          <Input
            style={{ border: "solid 1px #191F2D" }}
            type="radio"
            name="shipping"
            checked={selectedShipping === opcao}
            onChange={() => setSelectedShipping(opcao)}
          />
            ...
            <div className="d-flex justify-content-between align-items-center">
              <div>
                ...
                <div style={{ fontSize: "12px", color: "#666" }}>
                  {opcao === "PAC"
                    ? "Entrega em 6 dias úteis"
                    : opcao === "SEDEX"
                    ? "Entrega em 4 dias úteis"
                    : "Entrega em 3 dias úteis"}
                </div>
              </div>
              <div style={{ fontWeight: "500", color: opcao === "PAC" ? "#41DA69" : "#000" }}>
                {opcao === "PAC" ? "Grátis" : "R$ 24,90"}
              </div>
            </div>
          </div>
        </div>
